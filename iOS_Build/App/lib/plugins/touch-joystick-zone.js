    ...
    touchEnd: function(ev) {
        ...
        for (var i = 0; i < ev.changedTouches.length; i++) {
            if (ev.changedTouches[i].identifier === this.touchId) {
                ...
                this.touchStart.x = 0;
                this.touchStart.y = 0;
                console.log(this.curDelta);
                
                if(Math.abs(this.curDelta) > this.touchMinimumDelta){
                	this.complete = true;
                	if(this.curDelta < 0){this.dir="left"}
                	else {this.dir="right"}
    ...
    touchMove: function(ev) {
        ...
        for (var i = 0; i < ev.changedTouches.length; i++) {
            ...
            if (touch.identifier === this.touchId) {
                ...
                var winner = deltaX;
               // console.log(winner);
                if (Math.abs(winner) < this.touchMinimumDelta) {
                    continue;
                } else {
                	this.curDelta = winner;
